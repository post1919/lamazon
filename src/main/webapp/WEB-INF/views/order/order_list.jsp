		...
		<table cellspacing="0" cellpadding="0" class="clean_table search_form" summary="기본정보 입력양식입니다.">
			...
			<tbody>
				<tr>
					...
					<th scope="row"></th>
					<td>
						<a href="javascript:fn_list();" class="btnNormal red"><span>검색</span></a>
					</td>
				</tr>
				<tr>
					...
					<td>
						<c:if test="${IS_ADMIN}">
						<a href="javascript:fn_o_status(1000);" class="btnNormal orange"><span>미입고처리</span></a>
						<a href="javascript:fn_o_status(2000);" class="btnNormal green"><span>입고처리</span></a>
						<a href="javascript:fn_o_status(3000);" class="btnNormal blues"><span>출고처리</span></a>
						</c:if>
					</td>
				</tr>
			</tbody>
...
<c:if test="${ USER['U_TYPE'] eq '1000' }">
	<ul id="grid_context_menu" class="contextMenu" style ="display :none;">
		<li data-menu="cmenu_o_status_1000" class="cm_user cm_user_delete" target="_blank"><i class="fa"></i>미입고처리</li>
...
<script>
		...
		var grid_main_title = "운송관리";
	   	...
	   	$grid.pqGrid({
		    ...
		    , scrollModel   : {autoFit: true}
	   	    , selectionModel: { type: null } //다중선택 여부
		    , collapsible   : false
		    , dataModel     : {
				          		  location : "remote"   
						        , dataType : "JSON"
		    ...
		    , colModel  : [
		    		...
		    		{title:"이름",     dataType: "string" , width: "80",  dataIndx: "U_NAME",            align:"center",
		    			render: function (ui) {                         
	                        var rowData = ui.rowData["U_NAME"];
	                       	rowData = "<span style='font-weight:bold;'>"+rowData+"</span>";
	                        return rowData;
		    			}
		    		},
		    	    ...
		    	    {title:"주문번호", dataType: "string" , width: "120", dataIndx: "O_NUMBER",          align:"center",
		    	    	render: function (ui) {                         
	                        var rowData = ui.rowData["O_NUMBER"];
	                       	rowData = "<span style='font-weight:bold;'>"+rowData+"</span>";
	                        return rowData;
		    			}
		    	    },
	                {title:"제목",     dataType: "string" , width: "180", dataIndx: "O_TITLE",           align:"left"  },
					...
					<c:if test="${IS_ADMIN}">
					{title:"선택", dataIndx:"CHK_STATUS", cls:'ui-state-default', type:'checkBoxSelection', cb: {all:true, header:true}, align:"center", editable: false, resizable: false },
					</c:if>
		    ...
		    , cellClick : function( event, ui ) {
		    	//이름 / 주문번호만 링크
		    	if( ui.dataIndx == 'U_NAME' || ui.dataIndx == 'O_NUMBER' ){
			    	var rowIndx = ui.rowIndxPage;
					cRowIndx = rowIndx;
					...
					//주문번호
					if( ui.dataIndx == 'O_NUMBER' ){
						var url = "/order/detail?o_pk="+row['O_PK'];
						var openNewWindow = window.open("about:blank");
						openNewWindow.location.href = url;
						
					//이름
					} else if( ui.dataIndx == 'U_NAME' ){
	...
	function fn_o_status(o_status){
		...
		for(var i=0; i<data.length;i++){
			...
			if(row['CHK_STATUS']){
				console.log(i, " - ", row['CHK_STATUS'], " - ", row['O_PK']);
				checkedArr.push(row['O_PK']);
			}
		}
		...
		if( checkedArr.length == 0 ){
			...
			if( confirm("처리하시겠습니까?") ){
				var param = {
	               ...
	               data : { "o_status":o_status, "o_pks":checkedArr }
				}
	...
	function setUserRightClick() {
		...
		$("#grid_context_menu li").click(function() {
			...
			//입고처리
			if( datamenu == "cmenu_o_status_1000" || datamenu == "cmenu_o_status_2000" || datamenu == "cmenu_o_status_3000" ) {
				...
				if( datamenu == "cmenu_o_status_2000" ){
					proc_text = "입고";
					o_status  = 2000;
					
				} else if( datamenu == "cmenu_o_status_3000" ){
					proc_text = "출고";
					o_status  = 3000;
				}
