...
/* 레이어 팝 변수 */
function ViewlayerPop(pop_no, popup_partnername, popup_title, pr_pk){
	
	var WinHeight = $(window).height();
	
	var PopWidth = $("#layerPop"+pop_no).width();
	var pop_ = eval("document.getElementById('layerPop"+pop_no+"')");
	$("#popup_partnername").text( popup_partnername );
...
/* 레이어 팝 추가 2017.12.18 youn.jae.woong */
function ViewlayerPop2(pop_no, popup_partnername, popup_title, pr_pk, pr_id, pa_id){
	var WinHeight = $(window).height();
	
	var PopWidth = $("#layerPop"+pop_no).width();
	var pop_ = eval("document.getElementById('layerPop"+pop_no+"')");
	$("#popup_partnername").text( popup_partnername );
	$("#popup_partnername1").text( popup_partnername );
	$("#popup_title").text( popup_title );
...
/* gnb_sub */
	$(document).ready(function(){
		...
		var pageName2 = $(".sub_menuN > li.on > a").html();
		...
		var onGnb = $(".gnb ul > li.on > .sub_menuN").html();
		var onGnb2 = $(".gnb ul > li.on > .sub_menuN > li.on").html();
		$("#lnb_menu").html(onGnb);
		...
		$(".gnb > ul > li").mouseenter(function(){
			...
			item.children(".sub_menuN").stop().slideDown(100);
		});
		$(".gnb > ul > li").mouseleave(function(){
			var item = $(this);
			$("#gnb_bar").slideUp(200);
			//fadeTo( ①duration, ②opacity, [ ③easing ], [ ④callback ] );
			$(".sub_menuN").slideUp(200);
		});
		
		
		$(".gnb1").mouseenter(function(){
			var item = $(this);
			item.children(".drop_menu").slideDown(100);
		});
		$(".gnb1").mouseleave(function(){
			var item = $(this);
			$(".drop_menu").slideUp(100);
		});
